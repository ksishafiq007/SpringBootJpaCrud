...
public class StudentService {
	...
	public void saveOrUpdateStudent(StudentDTO studentDTO) {
		StudentEntity student =copyStudentDtoToStudent(studentDTO);		// BeanUtils.copyProperties(studentDTO, student);
	
		studentRepo.save(student);

	}
	
	public StudentEntity copyStudentDtoToStudent(StudentDTO studentDTO) {
		StudentEntity student = new StudentEntity();
		student.setStudentID(studentDTO.getStudentID());
		student.setStudentName(studentDTO.getStudentName());
		student.setEmail(studentDTO.getEmail());
		student.setStudentRoll(studentDTO.getStudentRoll());
		if(studentDTO.getPhotoName()!="") {
			student.setPhotoName(studentDTO.getPhotoName()); 
		}
		return student;
	}
